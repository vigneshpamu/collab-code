...
export const createFile = async (fileId: string) => {
  const res = await fetch(`https://storage.vigneshpamu2002.workers.dev/api`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
...
export const generateCode = async ({
  ...
  return await fetch(
    'https://api.cloudflare.com/client/v4/accounts/dfe11e43c5ce7c71a9b7ce503ac1e8d5/ai/run/@cf/meta/llama-3-8b-instruct',
    {
      method: 'POST',
      headers: {
        Authorization: 'Bearer pFVOnQSHBEjvxnxuY456dtXixW-gMWsRpKlkmJ7v',
        'Content-Type': 'application/json',
      },
      body: JSON.stringify({
        messages: [
...
export const getProjectSize = async (id: string) => {
  const res = await fetch(
    `https://storage.vigneshpamu2002.workers.dev/api/size?virtualboxId=${id}`
  )
...
export const getFolder = async (folderId: string) => {
  const res = await fetch(
    `https://storage.vigneshpamu2002.workers.dev/api?folderId=${folderId}`
  )
