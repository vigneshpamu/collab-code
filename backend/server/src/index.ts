...
io.use(async (socket, next) => {
  ...
  const dbUser = await fetch(
    `https://database.vigneshpamu2002.workers.dev/api/user?id=${userId}`
  )
  ...
  if (!Array.isArray(dbUserJSON.virtualbox)) {
    console.error('dbUserJSON.virtualbox is not defined or not an array')
    return
  }
  ...
  socket.on(
    ...
    async (
      ...
      const fetchPromise = fetch(
        `https://database.vigneshpamu2002.workers.dev/api/virtualbox/generate`,
        {
          method: 'POST',
          headers: {
