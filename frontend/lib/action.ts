...
export async function createVirtualbox(body: {
  ...
  const res = await fetch(
    'https://database.vigneshpamu2002.workers.dev/api/virtualbox',
    {
      method: 'PUT',
      headers: {
...
export async function deleteVirtualbox(id: string) {
  const res = await fetch(
    `https://database.vigneshpamu2002.workers.dev/api/virtualbox?id=${id}`,
    {
      method: 'DELETE',
    }
  )
...
export async function shareVirtualbox(virtualboxId: string, email: string) {
  try {
    const res = await fetch(
      'https://database.vigneshpamu2002.workers.dev/api/virtualbox/share',
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
...
export async function generateCode(code: string, line: number) {
  const res = await fetch(
    'https://api.cloudflare.com/client/v4/accounts/dfe11e43c5ce7c71a9b7ce503ac1e8d5/ai/run/@cf/meta/llama-3-8b-instruct',
    {
      method: 'POST',
      headers: {
        Authorization: 'Bearer oHZsvsnuZR5UQUBYychoRxXHLxEARE5zXA1ql5Mu',
        'Content-Type': 'application/json',
      },
      body: JSON.stringify({
        messages: [
